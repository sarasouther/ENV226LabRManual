...
| **Part 3 — The herbarium record** | A SEINet investigation of what herbarium records can and cannot tell you about where a plant grows, plus scoring real specimens against the standard from Part 2. | Always |
...
- [**Herbarium labels**](downloads/HerbariumLabel.docx) — print and cut out. One per specimen, filled in by hand and affixed to the sheet. Only needed if you collect.
- [**Specimen quality scoring sheet**](downloads/specimen_quality_datasheet.csv) — the checklist you score specimens against in Part 3.
...
4. Judge whether a herbarium specimen is good enough to be worth keeping.
5. Compare the tree community NAU planted with the tree community that grew here.
...
2. **Find the tree**, then key it as a group — one couplet at a time, out loud, so everyone sees which character decided it.
...
### Record it properly on the worksheet
...
### What makes a specimen worth keeping
...
**Label data.** Detailed enough that the place can be found again: GPS point, elevation, habitat and soil, date and collector. That is the next section, and it is where most specimens fail.
...
Download: [**Herbarium labels to print and fill in by hand**](downloads/HerbariumLabel.docx) — a worked example on the first page, blanks to cut out on the second.
...
### 6. Good or bad? Scoring real specimens

You have the standard from Part 2. Now use it on specimens you did not make.

Score three real digitized sheets against the checklist in [`specimen_quality_datasheet.csv`](downloads/specimen_quality_datasheet.csv). One row per criterion, scored 0, 1 or 2:

- **0** — absent or unusable
- **1** — present but compromised
- **2** — meets the standard

1. In SEINet, search for a species you collected or keyed this semester.
2. Open three records **with specimen images**, from three different collectors and, if you can, three different decades.
3. Score each one in the `SEINET-1`, `SEINET-2` and `SEINET-3` rows of the sheet, and note the catalogue number and collector for each. You cannot assess the paper stock from a photograph — score what you can see and mark the rest `NA`.
...
### If your group collected a specimen

Skip this section if you did not collect. If you did, do it once your specimen is dry.

**Score your own.** Run the same checklist over the specimen you pressed, using the `OWN` rows of the scoring sheet. Fifteen criteria at 0–2 gives a maximum of 30. Be hard on it: a specimen you scored honestly at 14 out of 30 is more useful to you than one you talked yourself into calling fine.

**Then swap.** Trade specimens with another group, score theirs in the `SWAP` rows, and compare their score of your specimen against your own score of it.
...
**Everything goes on one worksheet:** [`TreeWalk_Worksheet.docx`](downloads/TreeWalk_Worksheet.docx). Type into it and upload the file to Canvas. There is nothing else to hand in.

**Part A — the walk**

1. Your **tree table** — fifteen slots, at least ten of them filled: the pin name, the name you keyed it to, your confidence *recorded before you checked*, and whether you were right.
...
5. **The full record for one plant**, every row of the table filled in, plus the couplet that stopped you and the one observation that would settle it. Keep your photographs with the file.

**Part C — the record**

6. The five sections of the SEINet investigation from Part 3, including your **records-by-decade table**.
7. **Your three specimen scores** from section 6, and the single criterion that separated the best sheet from the worst.

**If your group collected and pressed a specimen**, add:

8. **Your own specimen's score**, the other group's score of it, and — if the two disagreed by more than 3 points — the criterion you read differently and which reading the herbarium would use.

**If you went on the guided walk for extra credit**, add:

9. **One character the guide used that is not in this manual's key**, why it works, and the couplet you would write to capture it.
