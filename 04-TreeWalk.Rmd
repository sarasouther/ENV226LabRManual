...
- [Tree walk datasheet (.csv)](downloads/tree_walk_datasheet.csv) — the field sheet; print it or keep it on a phone.
- [Unknown plant log (.csv)](downloads/unknown_plant_log.csv)
- [**Herbarium labels**](downloads/HerbariumLabel.docx) — print and cut out. One per specimen, filled in by hand. Only needed if you collect.
...
```
TreeNumber,PinName,Species,Confidence,Correct,Notes
```

- `TreeNumber` — 1, 2, 3… in the order you visit them
- `PinName` — the pin's label on the map, so a TA can tell which tree you mean
- `Species` — the scientific name you keyed to, or `UNK-1`
- `Confidence` — `High`, `Medium` or `Low`, decided **before** anyone opens the pin
- `Correct` — `Yes` or `No`, filled in *after* you open it
- `Notes` — anything the key did not use, or why it went wrong

You do not need to record whether the tree is native or planted. The key already marks every species **[N]** or **[P]**, so you can sort that out afterwards from your species list.
...
1. Your **tree table**: for each of at least ten trees, the pin name, the name you keyed it to, your confidence *recorded before you checked*, and whether you were right.
