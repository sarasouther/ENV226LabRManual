...
> **Skills this lab builds.** *4DEE:* Core Ecological Concepts → Landscapes. Cross-Cutting Themes → Space & Time. Ecology Practices → Quantitative reasoning and computational thinking. *BioSkills:* Modeling & Simulation → Purpose of Models; Model Application. Quantitative Reasoning → "Apply the tools of graphing, statistics, and data science to analyze biological data."
...
- [R script for creating the SDM](downloads/SpeciesDistributionModels.R)
- [Occurrence data](downloads/occurdata.csv)
- [Worksheet to turn-in](downloads/SDM_Questions.docx)
...
You do not need to download any climate data by hand. The script fetches the current bioclimatic variables and the future projection for you, using the `geodata` package, and caches them so they are only downloaded once. State boundaries come from `rnaturalearth` the same way. Expect the first run to take several minutes while those files download.
