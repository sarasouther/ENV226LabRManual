...
- **Street trees.** Identify, measure DBH, and score condition for every street tree along the transect. Test whether canopy or tree size differs between segments. The key from the tree walk lab and the DBH protocol from the population structure lab both apply directly.
...
- **Walk the route yourself first**, with attention to traffic, sidewalk gaps, and where 30 students can safely stand. Break into staggered groups rather than moving as one block.
- Twelve blocks is about right for the observation walk. The original module notes "usually about 12 blocks."
- **The conduct rules are not boilerplate.** Say them out loud before you leave, and enforce the no-photographing-people rule. This exercise is only defensible if it is done respectfully.
- Expect the proposal step to be where groups struggle — they will propose something un-executable or unfalsifiable. Budget real time for reviewing proposals; it is the highest-value teaching in the lab.
- The module runs comfortably over multiple weeks: week 1 walk and proposal, week 2 self-scheduled data collection, week 3 analysis and discussion, week 4 presentations. It also compresses into one lab period plus outside time if the slot is tight.
- Rubrics, a participation assessment and a background questionnaire ship with the original module and are worth using as-is. The module does **not** include a blank field datasheet — students design their own recording forms, which is intentional.
