...
<!--chapter:end:02-DataTypesDescription.Rmd-->
...
<!--chapter:end:03-BasicStatisticalTest.Rmd-->
...
<!--chapter:end:04-SelectingStatisticalTests.Rmd-->
...
<!--chapter:end:05-EcologicalSampling.Rmd-->
...
[Download the CSV file](https://drive.google.com/uc?export=download&id=167MbTJaHF4mL-8-50HvojooEPhtaO4mi)

[Download the R file](https://drive.google.com/uc?export=download&id=1Iokx1NopcYemVqavTaGVGrocOmM06q8E)
...
<!--chapter:end:06-NaturalSelection.Rmd-->
...
<!--chapter:end:07-PopulationEcology.Rmd-->
...
<!--chapter:end:08-InvasiveSpecies.Rmd-->
...
If you conduct the tests in the field, here is location information on where to meet [Download the map](https://drive.google.com/uc?export=download&id=1BGRB2hnSc7O_xM7g68wryQDFgTO3Sd4t)

[Download the excel file](https://docs.google.com/spreadsheets/d/19m-QEhLQ0SBdH81q0OG5gQtrhm1Z0C8x/export?format=xlsx) 
...
**GET NITRILE EXAM GLOVES FROM YOUR LAB INSTRUCTOR BEFORE HANDLING ANY CHEMICALS**
...
<!--chapter:end:09-WaterQuality.Rmd-->
...
<!--chapter:end:10-SpeciesInteractions.Rmd-->
...
<!--chapter:end:11-SpeciesDiversity.Rmd-->
...
![](/Users/sks379/Desktop/GraduateStudents/Stats and Lit Review Lessons/SpeciesDistributionModeling/SDM.png) 
...
![](/Users/sks379/Desktop/GraduateStudents/Stats and Lit Review Lessons/SpeciesDistributionModeling/NDimensionalSpace.png) 
...
#you can use this code to check out country codes
#geodata::country_codes()

#https://epsg.org/home.html website to look up crs codes
#‘World Geodetic System 1984’ (WGS 84) is a geodetic CRS with EPSG code 4326
#this is a common code, but we want to reduce the number of times that we reproject

#or save to your working directory
bio <- worldclim_global(var = "bio", version="2.1", res=2.5, path=tempdir())
...
biof6180  <- geodata::cmip6_world("CNRM-CM6-1", "585", "2061-2080", var="bioc", res=2.5, path=tempdir())
```
...
```{r, comment = NA, include = FALSE, message = FALSE, warning = FALSE, eval=FALSE}

species_name <- "Pinus ponderosa"
#gbif data are referenced by keys associated with particular species. let's get the key for Pediocactus simpsonii
key <- name_backbone(name=species_name, kingdom='plants', rank='species')$speciesKey

#by running the name_backbone function, we accessed GBIF to get the unique identifier and saved it as object 'key'. Now let's use that key to get occurrence data for this species.

#check out the occ_data function by typing ?occ_data in the command line. we can specify many fields to constrain what data we download. for our needs, we want data entries with a coordinate and that have not been flagged with a geospatial problem. Finally, we are primarily interested in North America. This is often important to specify, because people will post identifications from botanical gardens across the globe, which can yield weird results!

dat <- occ_data(taxonKey=key, hasCoordinate = TRUE, continent='north_america', limit=10000, hasGeospatialIssue = FALSE)$data

#select a few important columns, so that dataset isn't so crazy.
dat1 <- dat %>% dplyr::select(species, decimalLongitude, decimalLatitude, countryCode, gbifID, family, taxonRank, year, basisOfRecord)

names(dat1)[2:3] <- c("decimallongitude", "decimallatitude") #corrects coordinate name for cleaning
clean <- clean_coordinates(dat1, lon = "decimallongitude", lat = "decimallatitude", species = "species", countries = NULL, tests = c("capitals", "duplicates", "equal", "gbif", "seas", "zeros", "country", "centroids"), value = "clean", verbose = TRUE, report = FALSE)

prethindata <- clean[, c("decimallatitude", "decimallongitude", 'species')]
names(prethindata)[1:3] <- c("LAT", "LON", "SPEC")

#thin auto-correlated presence data
thinned_data <- spThin::thin(loc.data = prethindata, 
                  lat.col = "LAT", long.col = "LON",
                  spec.col = "SPEC",
                  thin.par = 30, reps = 100, #thinning 5km
                  locs.thinned.list.return = TRUE,
                  write.files=FALSE,
                  write.log.file=FALSE)

occurdata <- thinned_data[[1]]

#add a column to indicate occurrence 
occurdata$species <- 1

#change to a spatial point dataframe
occurdata2 <- st_as_sf(occurdata, coords = c("Longitude", "Latitude"))

#crs_wgs84 <- sf::st_crs(4326)  # WGS 84 has EPSG code 4326 
st_crs(occurdata2) <- 4326
st_crs(occurdata2) #check coordinate string
```
...
![](/Users/sks379/Desktop/GraduateStudents/Stats and Lit Review Lessons/SpeciesDistributionModeling/linear-regression-error-term.png) 
...
![](/Users/sks379/Desktop/GraduateStudents/Stats and Lit Review Lessons/SpeciesDistributionModeling/RandomForest.png) 
...
#reduce colinearity
geographic.extent <- extent(occurdata2) #could also use st_bbox
bioc <- crop(x = bio, y = geographic.extent)
ex <- terra::extract(bioc, occurdata2)
...
occurdata_df <- sf::as_Spatial(occurdata2)
...
```{r, include = FALSE, message = FALSE, warning = FALSE, eval=FALSE}
#choose modeling methods - to see all run code below
sdm::getmethodNames()
...
![](/Users/sks379/Desktop/GraduateStudents/Stats and Lit Review Lessons/SpeciesDistributionModeling/AUC.png) 
...
Submit your answers to the questions presented throughout this tutorial, your R script file, and the figures that you generated (Occurrence of *Pinus ponderosa* currently and in the future) to your TA.

<!--chapter:end:12-SpeciesDistributionModeling.Rmd-->
