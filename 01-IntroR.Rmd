...
Add your responses/tree data to this google sheet. [Open the class data sheet (add your data here)](https://docs.google.com/spreadsheets/d/1iWaYjmBsaiPl5_rwEaIEW4dU9rlZcbdt50cjVAsVExI/edit?usp=sharing)
...
**Save it as `WelcomeToENV226.csv`.** Your browser may add something to the name, like `WelcomeToENV226 - Sheet1.csv` — rename it so it matches exactly, or the R script will not find it.

If the class data isn't ready yet, you can run the script on an example file with the same columns: [example data (.csv)](downloads/WelcomeToENV226.csv)

Then, download your R script: [Download the R file](downloads/intro.R) Download and use the R script to practice with R.
...
### The full package list for this course

Different labs use different packages. Rather than installing them one at a time
mid-lab, run this block **once** at the start of the semester and you are set for
every chapter:
...
The species distribution modeling lab needs a larger set of spatial packages, and
its script installs those for you the first time you run it.

If a lab script stops with `there is no package called '...'`, you have found one
this list missed - install it and tell your instructor so it can be added.
