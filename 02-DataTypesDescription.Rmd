...
> **Skills this lab builds.** *4DEE:* Ecology Practices → Data analysis and interpretation; Quantitative reasoning and computational thinking. *BioSkills:* Quantitative Reasoning → "Perform basic calculations (e.g., percentages, frequencies, rates, means)"; "Create and interpret informative graphs and other data visualizations."

Over the next few weeks, we will build your skills evaluating and developing experiments, collecting data, analyzing data, and reporting results. We will start with something familiar to most folks in the class: descriptive statistics. Descriptive statistics refers to quantifying features of a focal population (e.g., describing the mean height of the adult population of the U.S., or the percent of a community that consists of flowering plants). Descriptive statistics aren’t statistical tests (which we will get to in the next tutorial), but they are a key part of the results section of a manuscript. They tell us what our data look like—summarizing central tendencies, variation, and overall patterns—so that we can see the “big picture” before diving into more complex analyses. In other words, descriptive statistics provide the foundation for interpreting data and for deciding what kinds of formal tests or models we might need next.

They are also an essential part of a complete results summary in a manuscript, which typically includes three components:

1.	The outcome of a statistical test,
2.	A description of the populations being studied and their responses to experimental treatments (often supported by descriptive statistics), and
3.	A visualization of the results (figures).

Over the next set of tutorials, we will build toward producing a results section that weaves these three components together, giving readers both a clear picture of the data and the evidence to support scientific conclusions.

## Objectives

In this lab, we will focus exclusively on data description and a teeny bit of visualization. 
...
- For your reference, here is a guide that shows the basics formulas for calculations in R: [Supplementary Material](downloads/calculator.pdf)

- [Download the example dataset](downloads/summarystatsdata.csv)
...
[R Script for Chapter 1](downloads/summarystatistics.R)
...
*Numerical variables* are variables that are numbers, and occur in two forms (discrete and continuous):
