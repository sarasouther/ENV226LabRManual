...
Now, let's play around a little more with R! For your reference, here is a guide that shows the basics formulas for calculations in R: [Supplementary Material](downloads/calculator.pdf)

Let's also try out a describing the petal lengths of 3 different plant species: Milkweed, Bluestar and Pectis using this dataset:
[Download the example dataset](downloads/lesson2data.csv)

Please download the data set and summarize the mean and standard error for each species, using this code:


Then, build on this code, but generating your own description of these species:
[R Script for Chapter 1](downloads/tutorial_script.R)

Please put your results in a document and submit on canvas!
